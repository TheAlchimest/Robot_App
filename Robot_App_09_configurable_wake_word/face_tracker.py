...
class EyeMovementController:
    ...
    # Arduino angles (إذا كنت تستخدم servo)
    ARDUINO_ANGLES = {
        'center': 90,
        'right': 60,
        'left': 120,
    ...
    def start_new_movement(self):
        ...
        # Arduino control
        if self.arduino:
            angle = self.ARDUINO_ANGLES[self.target_pos]
            try:
                self.arduino.sendData([0, 0, angle])
    ...
    def look_at_position(self, position: str):
        ...
        if position in self.POSITIONS:
            ...
            if self.arduino:
                angle = self.ARDUINO_ANGLES[position]
                try:
                    self.arduino.sendData([0, 0, angle])
                except Exception:
                    pass
...
def naturalEyeMovement(enable_arduino=False):
    """
    ...
    movement_ctrl = EyeMovementController(arduino)
...
def run_eyes_threaded(mode="natural", enable_arduino=False):
    """
    تشغيل العيون في thread منفصل
    mode: "natural" أو "tracking"
    """
    eye_state.set_mode(mode)
    
    if mode == "tracking":
        trackUserFace(enable_arduino)
    else:
        naturalEyeMovement(enable_arduino)
...
if __name__ == "__main__":
    ...
    run_eyes_threaded(mode, enable_arduino)